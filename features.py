...
class Features(object):
    def __init__(self, data_dir='flower_photos', image_size=150):
            ...
            for image_file in tqdm(os.listdir(image_dir)):
                if image_file[0] is not '.':
                    try:
                        ...
                        if flatten:
                            img = img.flatten()
                        label = self._create_labels(flower)
    ...
    def train_test_split(self, datasets, test_size=0.1):
    ...
    def _create_labels(self, flower):
